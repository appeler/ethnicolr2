...
![AppVeyor Build Status](https://ci.appveyor.com/api/projects/status/u9fe72hn8nnhmaxt?svg=true)
![PyPI Version](https://img.shields.io/pypi/v/ethnicolr.svg)
![Anaconda Version](https://anaconda.org/soodoku/ethnicolr2/badges/version.svg)
...
- Removes extra space
- For names in the [census file](ethnicolr/data/census/), it appends 
...
See [here](https://github.com/appeler/ethnicolr/blob/master/ethnicolr/data/census/census_2000.pdf) 
...
### pred_census_ln(df, namecol, year=2000, num_iter=100, conf_int=1.0)

**What it does:**

- Removes extra space.
- Uses the [last name census 2000 model](ethnicolr/models/ethnicolr_keras_lstm_census2000_ln.ipynb) or 
  [last name census 2010 model](ethnicolr/models/ethnicolr_keras_lstm_census2010_ln.ipynb) 
...
### pred_wiki_ln(df, namecol, num_iter=100, conf_int=1.0)

**What it does:**

- Removes extra space.
- Uses the [last name wiki model](ethnicolr/models/ethnicolr_keras_lstm_wiki_ln.ipynb) to
...
- Removes extra space.
- Uses the [full name wiki model](ethnicolr/models/ethnicolr_keras_lstm_wiki_name.ipynb)
  to predict the race and ethnicity.
...
**Output:** Appends the following columns to the pandas DataFrame or CSV:
race (categorical variable---category with the highest probability).
Categories same as pred_wiki_ln above.
For each race it will provide the mean, standard error, lower & upper
bound of confidence interval
...
### pred_fl_reg_ln(df, namecol, num_iter=100, conf_int=1.0)

**What it does:**

- Removes extra space, if there.
- Uses the [last name FL registration model](ethnicolr/models/ethnicolr_keras_lstm_fl_voter_ln.ipynb)
  to predict the race and ethnicity.
...
### pred_fl_reg_name(df, namecol, num_iter=100, conf_int=1.0)

**What it does:**

- Removes extra space.
- Uses the [full name FL model](ethnicolr/models/ethnicolr_keras_lstm_fl_voter_name.ipynb)
  to predict the race and ethnicity.
...
- Removes extra space, if there.
- Uses the [last name FL registration model](ethnicolr/models/ethnicolr_keras_lstm_fl_voter_ln_five_cat.ipynb)
  to predict the race and ethnicity.
...
- Removes extra space.
- Uses the [full name FL model](ethnicolr/models/ethnicolr_keras_lstm_fl_voter_ln_five_cat.ipynb)
  to predict the race and ethnicity.
...
- Removes extra space.
- Uses the [full name NC model](ethnicolr/models/ethnicolr_keras_lstm_nc_12_cat_model.ipynb)
  to predict the race and ethnicity.
...
**Output:** Appends the following columns to the pandas DataFrame or CSV:
race + ethnicity. The codebook is [here](https://github.com/appeler/nc_race_ethnicity). For each
...
In particular, we utilize the last-name--race data from the [2000 census](http://www.census.gov/topics/population/genealogy/data/2000_surnames.html)
and [2010 census](http://www.census.gov/topics/population/genealogy/data/2010_surnames.html),
...
the [Contributor Code of Conduct](http://contributor-covenant.org/version/1/0/0/).
